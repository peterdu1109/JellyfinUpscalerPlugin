...
namespace JellyfinUpscalerPlugin
{
    ...
    public class UpscalerCore
    {
        ...
        public async Task<HardwareProfile> DetectHardwareAsync()
        {
            ...
            _logger.LogInformation("🔍 Detecting hardware capabilities for AV1 support...");
            ...
            try
            {
                ...
                _logger.LogInformation($"✅ Hardware Profile: {profile.GpuVendor} {profile.GpuModel}, AV1: {profile.SupportsAV1}");
        ...
        private async Task DetectGpuCapabilities(HardwareProfile profile)
        {
            try
            {
                ...
                _logger.LogInformation($"📊 Detected GPU: {profile.GpuVendor} {profile.GpuModel}");
                _logger.LogInformation($"🔧 Available HW Accels: {string.Join(", ", profile.AvailableHwAccels)}");
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ GPU detection via FFmpeg failed");
            }
        }
        ...
        private async Task DetectNvidiaGpu(HardwareProfile profile)
        {
            try
            {
                    ...
                    profile.GpuModel.Contains("A5000") || profile.GpuModel.Contains("A6000"))
                {
                    ...
                    _logger.LogInformation($"🔥 NVIDIA AV1 Hardware Support Detected: {profile.GpuModel}");
                }
                else
                {
                    _logger.LogInformation($"ℹ️ NVIDIA GPU without AV1 support: {profile.GpuModel}");
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ NVIDIA specific detection failed");
            }
        }
        ...
        private async Task DetectIntelGpu(HardwareProfile profile)
        {
            try
            {
                ...
                if (output.Contains("av1_qsv"))
                {
                    ...
                    _logger.LogInformation($"🔥 Intel Arc AV1 Hardware Support Detected!");
                }
                else
                {
                    profile.GpuModel = "Intel Graphics (No AV1)";
                    _logger.LogInformation($"ℹ️ Intel GPU without AV1 support");
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ Intel GPU detection failed");
            }
        }
        ...
        private async Task DetectAmdGpu(HardwareProfile profile)
        {
            try
            {
                ...
                if (output.Contains("av1_vaapi") || output.Contains("av1_amf"))
                {
                    ...
                    _logger.LogInformation($"🔥 AMD AV1 Hardware Support Detected!");
                }
                else
                {
                    profile.GpuModel = "AMD Graphics (No AV1)";
                    _logger.LogInformation($"ℹ️ AMD GPU without AV1 support");
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ AMD GPU detection failed");
            }
        }
        ...
        {
            if (!profile.SupportsAV1) return;
            
            try
            {
                _logger.LogInformation("🧪 Testing AV1 encoding capability...");
                ...
                if (process.ExitCode == 0 && File.Exists(testOutput))
                {
                    ...
                    _logger.LogInformation("✅ AV1 encoding test successful!");
                }
                else
                {
                    profile.Av1TestPassed = false;
                    profile.SupportsAV1 = false;
                    _logger.LogWarning("❌ AV1 encoding test failed, disabling AV1 support");
                }
            ...
            {
                _logger.LogWarning(ex, "⚠️ AV1 capability test failed");
                profile.Av1TestPassed = false;
            }
        }
        ...
        private void ApplyHardwareOptimizations(HardwareProfile profile)
        {
            ...
            if (profile.SystemRamMB < 8192 || profile.VramMB < 4096)
            {
                ...
                _logger.LogInformation("💡 Light Mode recommended for this system");
            }
            else if (profile.SupportsAV1 && profile.VramMB >= 8192)
            {
                ...
                _logger.LogInformation("🚀 High-performance mode available");
            }
            else
            {
            ...
            try
            {
                ...
                if (profile.SystemRamMB < 4096 && profile.CpuCores <= 4)
                {
                    ...
                    _logger.LogInformation("📱 Mobile device detected (low RAM/CPU), enabling battery optimization");
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ Mobile detection failed, assuming desktop");
            }
        }
    ...
    /// <summary>
    /// Hardware profile with real capabilities
    /// </summary>
    public class HardwareProfile
    {
        ...
        public string Av1Encoder { get; set; } = "";
        public string Av1Decoder { get; set; } = "";
        public string MaxAV1Resolution { get; set; } = "1080p";
        public bool Av1TestPassed { get; set; }
